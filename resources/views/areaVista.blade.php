    ...
    <div class="p-4 sm:ml-64">
        <div class="p-4 bg-white shadow-2xl rounded-2xl mt-[2rem]">
            ...
            <div class="grid grid-cols-3 mt-[3rem] gap-4">
                <select id="filtroTipoArea" id="tipoArea" name="fkTipoArea" class="p-2 border border-gray-300 rounded-md placeholder:font-light placeholder:text-gray-500" required>
                    <option value="">Selecciona un tipo de área</option>
                    @foreach ($datosTipoArea as $dato)
                        <option value="{{$dato->nombreTipoArea}}">{{$dato->nombreTipoArea}}</option>
                    @endforeach
                </select>

                <select id="filtroEdificio" id="edificio" name="fkEdificio" class="p-2 border border-gray-300 rounded-md placeholder:font-light placeholder:text-gray-500" required>
            ...
            <div class="mt-10">
                <h2 class="mb-3 text-4xl font-bold text-center">Áreas registradas</h2>

                <table class="w-full border-4 border-gray-300 table-auto" id="tabla-areas" class="display nowrap" width="100%">
                    <thead class="border-4 bg-gray-200 border-gray-300 text-center">
                        <tr class="h-24">
                            <th class="linea-izquierda border-gray-300">Nombre/número del área</th>
                            <th class="linea-izquierda border-gray-300">Tipo de Área</th>
                            <th class="linea-izquierda border-gray-300">Edificio</th>
                            <th class="linea-izquierda border-gray-300">Acciones</th>
                        </tr>
                    </thead>
                    @foreach ( $datosArea as $dato )
                        <tr class="h-20 text text-center linea-abajo border-gray-300">
                            <td class="linea-izquierda border-gray-300">{{ $dato->nom_numArea }}</td>
                            <td class="linea-izquierda border-gray-300">{{ $dato->tipoArea->nombreTipoArea}}</td>
                            ...
                            <td class="linea-izquierda border-gray-300 flex items-center justify-center gap-3">
                                <a href="{{route('area.actualizado', $dato->pkArea)}}" class="mt-4">
                                ...
                                <a href="{{route('area.baja', $dato->pkArea)}}" class="mt-4">
                                    <svg class="w-[2rem] h-[2rem]" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
