    ...
    <div class="relative flex flex-col m-6 space-y-8 bg-white shadow-2xl rounded-2xl md:flex-row md:space-y-0">
      ...
      <div class="flex flex-col justify-center p-8 md:p-14">
        ...
        <form class="formulario123" action="{{route('edificio.insertar')}}"  method="post">
          ...
          <h2 class="mb-3 text-4xl font-bold">Agregar edificio</h2>
          <div class="py-4">
            <label class="mb-2 text-md" for="nombreEdificio">Nombre del edificio</label> 
            <input type="text" name="nombreEdificio" class="w-full pr-10 pl-10 p-2 border border-gray-300 rounded-md placeholder:font-light placeholder:text-gray-500" required>
          </div>

          <div class="py-4">
            <label class="mb-2 text-md" for="pseudonimo">Pseudonimo</label> 
            <input type="text" name="pseudonimo" class="w-full pr-10 pl-10 p-2 border border-gray-300 rounded-md placeholder:font-light placeholder:text-gray-500" required>
          </div>

          <div class="flex justify-center items-center pt-4">
            <input type="submit" value="Agregar" class="w-fit pr-9 pl-9 bg-gray-900 text-white p-1 rounded-sm mb-3 hover:bg-white hover:text-gray-900 hover:border hover:border-gray-300">
          </div>
        </form>
        ...
        <div class="mt-10">
          <h2 class="mb-3 text-4xl font-bold">Edificio registrados</h2>

          <form action="{{ route('edificio.mostrar') }}" method="GET">
            <div class="grid grid-cols-2 mt-[3rem] gap-4">
              <label class="mb-2 text-md">Edificio</>
              <input class="mb-2 text-md" type="search" name="search" value="{{ request()->query('search') }}" placeholder="Buscar por nombre">
              <input type="submit" value="Buscar" class="w-fit pr-9 pl-9 bg-gray-900 text-white p-1 rounded-sm mb-3 hover:bg-white hover:text-gray-900 hover:border hover:border-gray-300">
            </div>
          </form>

          <table id="tabla-edificio" class="w-full border-4 border-gray-300 table-auto" class="display nowrap" width="100%">
            <thead class="border-4 bg-gray-200 border-gray-300 text-center">
              <tr class="h-24">
                <th class="linea-izquierda border-gray-300">Nombre</th>
                <th class="linea-izquierda border-gray-300">Pseudonimo</th>
            ...
            @foreach($datosEdificio as $dato)
              <tr class="h-20 text text-center linea-abajo border-gray-300">
                <td class="linea-izquierda border-gray-300">{{$dato->nombreEdificio}}</td>
                <td class="linea-izquierda border-gray-300">{{$dato->pseudonimo}}</td>
                <td class="linea-izquierda border-gray-300">
                  <div>
                    <div>
                        <a href="{{route('edificio.mostrarId', $dato->pkEdificio)}}">
                    ...
                    <div>
                      <a href="{{route('edificio.baja', $dato->pkEdificio)}}">
                        <svg class="w-[2rem] h-[2rem]" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
